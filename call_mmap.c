...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4) 
    {
        fprintf(stderr,"Usage: exe_name python_file class_name function_name\n");
        return 1;
    }
	...
	indata = (char *) MapViewOfFile (map, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, 0); 
#else
	int fd;
...
#else
	// POSIX code
    pthread_create( &mythread, NULL, myThread, (void*)&cmd );
#endif
    ...
    printf("Main thread waiting for Python thread to complete...\n");
